...
productController.createProduct = async (req, res, next) => {
  ...
  let result;
  try {
    if (!name || !price || !stock) throw new Error('missing info when create product');
...
productController.getAllProduct = async (req, res, next) => {
  ...
  return sendResponse(res, 200, true, {result, count}, false, 'Successfully get all products');
};
...
productController.updateProduct = async (req, res, next) => {
  ...
  return sendResponse(res, 200, true, result, false, 'Successfully update product by Id');
};
...
productController.deleteProduct = async (req, res, next) => {
  ...
  try {
    if (!productId) throw new Error('product not found, deleted');
    result = await Product.findByIdAndUpdate(productId, {isDeleted: true}, {new: true});
  } catch (error) {
    return next(error);
  }
...
productController.getSingleProduct = async (req, res, next) => {
  ...
  return sendResponse(res, 200, true, result, false, 'Successfully get single product');
};
